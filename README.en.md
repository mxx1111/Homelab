...
SMART — twelve collectors on a single page. And not just watching: ban an IP,
...
- [Getting Started](#getting-started)
  ...
  - [Step 5: Unlock write operations](#step-5-unlock-write-operations)
...
- Multi-node clusters — this is a single-host panel with one read-only remote collector
- Long-term high-resolution metrics — history is minute-level sampling; use Prometheus
- Multi-user access control — there is no login, see [Security notes](#security-notes)
...
| Tab | What's in it |
|---|---|
| **Overview** | Security posture, storage, host load, network, service health, port exposure, containers, drives, certificates |
...
Twelve collectors: `host` `network` `containers` `services` `crowdsec` `storage`
...
### Step 5: Unlock write operations
...
firewall and control your containers, yet it has no login of its own.

Two ways to unlock, pick one:

**Option A: set an operation token (recommended)**

```yaml
# config.yaml
firewall:
  write_token: "some-random-string"
```
...
The UI grows a token field; enter it once and it's kept in browser localStorage.

**Option B: declare the network trusted**
...
Only do this when the dashboard is genuinely reachable from your LAN or VPN only.
...
| `firewall` | Write switch and token, see [Step 5](#step-5-unlock-write-operations) |
...
**This dashboard has no login.** It aggregates the state of an entire machine on
one page — service inventory, exposed ports, container list, internal topology —
which is exactly the intelligence an attacker wants. Add firewall and container
control on top and exposing it to the internet is a disaster.

The tradeoffs it makes:

1. **Writes are refused by default.** With no `write_token` configured, every
   write endpoint returns 403. Either set a token or explicitly declare
   `allow_anonymous_write: true`.
2. **Protected networks can't be banned.** Private ranges (`10/8`, `172.16/12`,
...
3. **Snapshot deletion only generates a command.** Volumes are mounted read-only;
   the dashboard hands you a command to run on the host yourself. Granting
   data-deletion rights to an unauthenticated panel isn't worth it.
4. **Only writes are audited.** The UI polls every 5 seconds — logging GETs would
...
- If you must expose it, put an authenticating reverse proxy in front
  (Authelia, Cloudflare Access, …) rather than relying on `write_token` alone
...
Writes are locked by default, see [Step 5](#step-5-unlock-write-operations).
...
```
12 collectors (independent loops) ─┬─ in-memory cache ─→ FastAPI ─→ frontend (5s poll)
...
```
backend/
  main.py            FastAPI routes and audit middleware
...
rules, drive SMART monitoring, ISP name normalization.

Not yet:

- Per-container detail page — historical resource usage for a single container
- Real snapshot deletion — currently generates a command; needs authentication first
